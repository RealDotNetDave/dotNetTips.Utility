...
' ***********************************************************************
' <copyright file="WebHelper.vb" company="David McCarter Consulting">
...
Namespace Web
  ...
  Public Module WebHelper
    ...
    ''' <returns></returns>
    ''' <remarks>
    ''' Original Code By: Aslam.Iqbal@electoralreform.co.uk
    ...
    ''' </summary>
    ''' <returns></returns>
    ''' <remarks>Strips out "sensitive" values stored in the HttpHeadersToIgnore resourse.</remarks>
    ...
    ''' <returns></returns>
    ''' <remarks>Strips out "sensitive" values stored in the HttpHeadersToIgnore resourse unless includeSensitiveValues is true.</remarks>
    Friend Function ExtractIncomingRequestCustomHeaderValues(includeSensitiveValues As Boolean) As IReadOnlyDictionary(Of String, String)
      ...
      For Each key In items.AsParallel()
        ...
        If includeSensitiveValues = False AndAlso key.ToLower().ContainsAny(My.Resources.HttpHeadersToIgnore.Split(dotNetTips.Utility.Portable.ControlChars.Comma)) Then
          value = If(String.IsNullOrWhiteSpace(currentRequest.Headers(key).ToString()), False.ToString(), True.ToString())
        Else
          value = currentRequest.Headers(key).ToString()
        End If
    ...
    Public Function JsonEncode(ByVal input As Object) As String
      ...
      Contract.Ensures(Contract.Result(Of String)().IsNotNull)
